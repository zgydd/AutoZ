...
;~ 	  
	  ...
	  FileClose($search)
	   FileCopy ( "C:\log\OfficeScanVersion.log", $sLocalName & "\", 1)
	   FileWriteLine(@ScriptDir& "\over.txt","test over!")  
       FileCopy ( @ScriptDir& "\over.txt", $sLocalName & "\", 1)
	  If $iscpy = False Then
		 _FileWriteLog(@ScriptDir & "\UpLoadLogToServer.log","Log not copy to server!If now you are in server,Please ignore this message.")
	  EndIf
